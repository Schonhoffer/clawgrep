...
use clawgrep::index::{discover_files, CHUNK_LINES, CHUNK_OVERLAP};
...
fn keyword_exact_substring_match() {
    ...
    let hits = keyword_search("012345678901", &files, CHUNK_LINES, CHUNK_OVERLAP);
...
fn keyword_no_match_returns_empty() {
    ...
    let files = vec![dir.path().join("a.txt")];
    let hits = keyword_search("xyzzy_nonexistent", &files, CHUNK_LINES, CHUNK_OVERLAP);
    assert!(hits.is_empty());
}
...
fn keyword_partial_word_match() {
    ...
    .unwrap();
    let files = vec![dir.path().join("a.txt")];
    let hits = keyword_search("database error", &files, CHUNK_LINES, CHUNK_OVERLAP);
...
fn keyword_stemming_works() {
    ...
    let hits = keyword_search("connection", &files, CHUNK_LINES, CHUNK_OVERLAP);
    assert!(
        !hits.is_empty(),
...
fn keyword_regex_match() {
    ...
    let files = vec![dir.path().join("a.txt")];
    let hits = keyword_search(r"[A-Z]+-\d+-[A-Z]+", &files, CHUNK_LINES, CHUNK_OVERLAP);
    assert!(!hits.is_empty());
...
fn keyword_search_on_project_files() {
    ...
    let hits = keyword_search("barcode UPC", &files, CHUNK_LINES, CHUNK_OVERLAP);
    assert!(!hits.is_empty());
...
fn keyword_search_scores_descending() {
    ...
    make_project(dir.path());
    let files = discover_files(dir.path(), &test_discover_opts());
    let hits = keyword_search("barcode", &files, CHUNK_LINES, CHUNK_OVERLAP);
...
fn keyword_empty_query() {
    ...
    let files = vec![dir.path().join("a.txt")];
    let hits = keyword_search("", &files, CHUNK_LINES, CHUNK_OVERLAP);
